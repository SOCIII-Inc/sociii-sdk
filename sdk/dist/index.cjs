...
__export(src_exports, {
  ...
  Sociii: () => Sociii,
  SociiiError: () => SociiiError,
  default: () => src_default
});
module.exports = __toCommonJS(src_exports);
var SociiiError = class extends Error {
...
var AuthenticationError = class extends SociiiError {
  constructor(message) {
    super(message || "Authentication required", "UNAUTHORIZED", 401);
    this.name = "AuthenticationError";
  }
};
var RateLimitError = class extends SociiiError {
...
var NotFoundError = class extends SociiiError {
  constructor(message) {
    super(message || "Resource not found", "NOT_FOUND", 404);
    this.name = "NotFoundError";
  }
};
var DEFAULT_BASE_URL = "https://api-feyfibglbq-uc.a.run.app";
var MAX_RETRIES = 3;
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (res.status === 429) {
        ...
        const waitMs = retryAfter ? parseInt(retryAfter, 10) * 1e3 : INITIAL_BACKOFF_MS * Math.pow(2, attempt);
        if (attempt < retries) {
          await sleep(waitMs);
          continue;
        }
        const body2 = await safeJson(res);
        throw new RateLimitError(body2?.error || "Rate limit exceeded", retryAfter);
      }
...
var WorkersClient = class {
  ...
  async get(slugOrId) {
    if (!slugOrId)
      throw new SociiiError("slugOrId is required", "MISSING_FIELDS");
  ...
  async chat(workerId, message, options = {}) {
    ...
    if (!message)
      throw new SociiiError("message is required", "MISSING_FIELDS");
    const body = {
      message,
      workerId,
